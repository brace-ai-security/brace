...
- [ ] **B08 · G2 · C1 — Environment isolation.** Document the agent's reachable systems and data. Test that it has no route to unauthorized production resources, the admin plane, host services, or other tenants. Enforce isolation through the infrastructure, not solely through prompt instructions.
...
- [ ] **R01 · G2 · C5 — Validate every input boundary.** Inventory user input, fetched pages, files, tool outputs, retrieved chunks, and peer messages. Apply type, schema, size, and content checks before use. Exercise malformed, oversized, and adversarial inputs, including those from internal systems.
- [ ] **R02 · G2 · C5 — Separate data from instructions.** Keep external content out of trusted system and policy channels; label its source and trust level. Test injection through documents and tool output. Channel separation is a mitigation, so also verify that execution controls contain a model that follows the injected instruction.
