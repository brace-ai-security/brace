...
For the difficult checks, read the **operational challenge and practical fallback notes directly beneath the corresponding checklist item**. Use the [operational limits guidance](CHECKLIST.md#working-with-operational-limits) to record a bounded claim, evidence, uncertainty, and failure response. Exercise the chosen fallback as well as the nominal path; these recipes alone do not establish production effectiveness.
...
| B09 | Call one allowed and one prohibited test destination, including direct-server/proxy-bypass routes, redirect chains, DNS changes, IPv6, and controlled metadata/loopback fixtures. Test an unauthorized resource on an allowed service too. Only the allowed route works; the denial reaches the designated operator within the alert budget. | Egress rules, gateway/network logs, alert receipt time. |
