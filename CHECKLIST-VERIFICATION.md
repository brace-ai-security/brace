...
Use this alongside the [sign-off checklist](CHECKLIST.md). Each recipe gives a procedure, an expected result, and the evidence to retain under the matching checklist item ID. The checklist defines the requirements and priority gates; these recipes help you verify them and do not replace them.

For the difficult checks, read the **operational challenge and practical fallback notes directly beneath the corresponding checklist item**. Use the [operational limits guidance](CHECKLIST.md#working-with-operational-limits) to record what the control can demonstrate, its evidence and limitations, and how failures are handled. Test the fallback as well as normal operation; these recipes alone do not establish production effectiveness.

The [source review](SOURCE-REVIEW.md) distinguishes primary guidance, BRACE-specific policy, and product claims that still need deployment evidence.
...
Each recipe provides a starting point. Repeat it for each materially different integration, permission boundary, and execution path in scope. Passing a finite set of injection examples does not prove that all prompt injection is prevented.
...
|---|---|
| Role and owner | The model’s job and the team responsible for selecting and updating it. Match this to the running workflow, including fallback models and routing rules. |
...
| B02 | With the agent's actual test credentials, perform an allowed read, prohibited write/admin action, and request for tenant B's resource from tenant A. The allowed read must succeed; all prohibited requests must fail. Inspect the destination for side effects. | Token scope metadata without token values, authorization logs, resource-state checks. |
| B03 | Issue a short-lived test credential, call before and after expiry, rotate it, and retry the old credential. The expired credential fails; the retired credential fails after any explicitly permitted overlap, measured against policy. Inspect image layers, prompt artifacts, and repository secret-scan results for standing secrets. | Issuance/expiry times, rotation policy, request results, redacted scan results. |
| B04 | Open a session, revoke the agent identity's access, then retry within the declared revocation interval through both existing and new sessions. Check self-contained/cached tokens, refresh-token issuance, and existing sessions. Agent requests fail within the budget through trusted enforcement; unrelated user requests still succeed. | Revocation event, session results and timestamps. |
...
| B06 | Approve a test action, then alter its target or arguments; separately expire approval, use an unauthorized reviewer, and disable the approval service. Each invalid attempt fails. Race two submissions with the same approval and change relevant destination state after approval. Only authorized uses of the bound action may execute; replay and invalid state are rejected. | Approval payloads, reviewer roles, gate decisions, resulting state. |
...
| B09 | Call one allowed and one prohibited test destination, including direct-server/proxy-bypass routes, redirect chains, DNS changes, IPv6, and controlled metadata/loopback fixtures. Test an unauthorized resource on an allowed service too. The authorized request must succeed. Prohibited requests must fail, and the denial must reach the designated operator within the alert time limit. | Egress rules, gateway/network logs, alert receipt time. |
| B10 | Deploy the approved signed digest, then an unsigned image, an image signed by an unapproved identity, and a changed digest. Admission permits only the approved artifact. Verify the running digest matches the signed build record. | Signature/admission results, build provenance, running image digest. |
| B11 | Inspect installed binaries, mounts, privileges, and sandbox configuration. From the agent runtime, attempt access to a harmless host-only marker and an unmounted test path. Both fail; required task paths still work. Review the sandbox design/configuration or managed-provider evidence separately: these probes alone do not prove kernel-escape resistance. | Runtime configuration, binary inventory, access results. |
...
| R03 | Return a recognizable synthetic secret in tool stdout, debug output, and an API error. Trigger a normal run and search model inputs, ordinary logs, and external responses for the marker. It must be removed or appropriately restricted before those boundaries. Also feed generated SQL/shell/HTML payloads into downstream consumers and verify parameterization/escaping prevents unintended execution. | Marker test, redaction decisions, searches across each output surface. |
| R04 | Create searchable test documents for tenants A/B with unique markers. Attempt ingestion using an unauthorized identity: deny it. Query as tenant A and verify that no chunks belonging to tenant B are returned. Then quarantine a poisoned fixture, expire a stale fixture, and revoke access to another document. After the declared propagation interval, exact-marker and semantic searches plus cache checks must return none of the affected chunks. | Ingestion permissions and rejection; chunk source/time/trust/ACL records; before/after searches and cache invalidation timestamps. |
...
| R07 | Capture short- and long-context decisions and a tool handoff. Compare emitted context-size values with the harness's documented counting method and timing, including units and any estimation limits. Include cached-input and compacted-context cases. Every action must have a trace-linked, queryable record of the value and how it was measured; unknown context must be flagged, not reported as zero. | Counting definition, model-request metadata, trace query. |
...
| R12 | Stop runs before a write, during a transaction, and after a committed external test action. Inspect queues and destination state. Each case must have a defined canceled, completed, or reconciled outcome, or a tracked reconciliation hold with an owner and no dependent actions or blind retries; no unexplained orphan/restart is acceptable. | Scenario outcomes, queue snapshots, compensation/reconciliation records. |
| R13 | Execute a known sequence with an allowed call, denial, approval, tool error, and retry. Rebuild its graph using stored audit records only and compare with the test runner and destination logs. Every expected action must be accounted for; unresolved destination outcomes must be explicit and dependent work paused pending reconciliation. | Expected event list, reconstructed graph, missing-event query. |
| R14 | Modify a copy of a checkpoint/audit record: recovery must reject it. Also truncate, roll back, and replace the chain/checkpoint; verification must use independently protected trust state. For integrations supporting safe automatic replay, replay an intact checkpoint twice; the logical action occurs once with recorded duplicate suppression. For unsupported integrations, inject an ambiguous result and verify replay pauses for destination reconciliation without issuing a duplicate. | Integrity-verification result, destination-enforced idempotency results, action counts, manual-recovery hold and resolution. |
...
| E03 | Load an approved tool, then change its description/schema or implementation version in a test server and refresh. Unexpected changes must block use pending review. Verify a metadata hash does not falsely claim to detect changed remote code with unchanged metadata. Record implementation pinning/visibility separately; query affected dependency consumers. | Approved/new fingerprints, load denials, affected-deployment query. |
...
| E06 | Perform allowed calls through the gateway, then try direct-server bypass, wrong-audience/expired tokens, malformed responses, and a scanner/policy outage. Exercise downstream credential separation and local stdio launch restrictions where applicable. Unauthorized and unchecked traffic fails closed; allowed traffic remains audited and rate limits take effect. | Route/policy configuration, request/response fixtures, gateway decisions. |
| E07 | Send valid, malformed, forged-sender, replayed, and cross-tenant peer requests; reused action requests must not repeat side effects without authorization. Authenticate and scope even validly signed requests; deny disallowed requests and retain hostile content as untrusted data. Include a schema-valid injected instruction. | Peer policy, message fixtures, authorization and validation logs. |
